...
window.onload = function () {
  ...
  if (!dados) {
    mostrarAviso(t("aviso_erro_carregar_draft"));
    return;
  }
...
function criarDropZone(playerIndex, tipo) {
  ...
  titulo.innerText =
    tipo === "idol" ? t("board_idols") :
    tipo === "music" ? t("board_musica") :
    t("board_produtor");
...
function encerrarTurno() {
  if (!escolhaDoTurno) {
    mostrarAviso(t("aviso_escolha_card"));
    return;
  }
  let player = jogadorAtual();
  //trava picks
...
function atualizarTurnoUI() {
  if (turno >= ordem.length) {
    document.getElementById("turnoAtual").innerText = t("turno_encerrado");
    return;
  }
  let index = jogadorAtual();
  document.getElementById("turnoAtual").innerText =
    t("turno_de") + jogadores[index];
}
...
function getCardImageCandidates(item) {
  ...
  candidates.push(`../assets/draft-cards/${itemType}_${idSlug}.jpeg`);
  ...
  img.onerror = () => {
    ...
    if (fallbackType === "idol") {
      const fallbackGroup = item.group
        ...
        .replace(/\s+/g, "_")
        .replace(/[^a-zA-Z0-9_\-]/g, "");
      img.src = `../assets/draft-cards/idol_${fallbackGroup}_${fallbackName}.jpg`;
    } else {
      img.src = `../assets/draft-cards/${fallbackType}_${fallbackName}.jpg`;
    }
  };
...
function encerrarDraft() {
  ...
  mostrarAviso(t("aviso_draft_encerrado"));
  ...
  if (btn) {
    btn.innerText = t("btn_fase_simulacao");
    btn.style.backgroundColor = "#7e22ce";
    btn.onclick = () => { 
      const simulacaoHref = pegarHrefJogo("simulation-page.html");
